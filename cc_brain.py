...
def _build_context(mode: str = "fast") -> str:
    """构建当前上下文。mode: "fast"=快速回答, "deep_intro"=深度概述, "deep_detail"=深度展开"""
    from datetime import datetime
    ...
    if mode == "deep_intro":
        # Gemini：复杂问题的快速判断（有料，不是空泛概述）
        system += (
            "\n川哥问了一个有深度的问题。"
            "\n给出你的核心判断和关键结论，2-4句话，要有干货。"
            "\n不要空泛开场（不要说'这个问题很有意思'、'让我想想'）。"
            "\n直接切入要点，像一个懂行的朋友快速给你划重点。"
        )
    elif mode == "deep_detail":
        # MiniMax：系统化深度研究
        system += (
            "\n你正在做一个深度研究回答，前面已经给了核心判断。"
            "\n现在系统化展开：补充论据、分析不同角度、给出具体案例。"
            "\n不要重复前面说过的结论，直接进入新的深度内容。"
            "\n用自然的衔接词接上（'展开来看'、'具体来说'、'另一个角度'）。"
            "\n每个要点 2-3 句，有条理有深度，像同一个人在继续深入讲。"
        )
    else:
        # Gemini：日常快速回答
        system += (
            "\n根据问题复杂度自适应回答长度：简单的 1-2 句，需要解释的 3-5 句。"
            "\n第一句就是答案或观点，不要铺垫。"
            "\n川哥问你看到什么时，用自然的话描述画面。"
        )
...
def _stream_gemini_proxy(user_text: str, mode: str = "fast") -> Generator[str, None, None]:
    """Gemini 2.5 Flash 代理流式"""
...
def _stream_gpt_proxy(user_text: str) -> Generator[str, None, None]:
    """GPT 5.4 代理流式（云端深度展开，SSE stream）"""
    config = _load_gpt_proxy_config()

    system_prompt = _build_context("deep_detail")
    messages = [{"role": "system", "content": system_prompt}]
    for msg in _history[-MAX_HISTORY:]:
        role = "user" if msg["role"] == "user" else "assistant"
        messages.append({"role": role, "content": msg["text"]})

...
_TOOL_PATTERNS: list[tuple[str, str, int]] = [
    ...
    (r"(?:谁唱的|谁的歌|哪首歌)", "music_state", 0),
    # 停止/关闭（高优先级，避免被 smart 吞掉）
    (r"(?:暂停|停止|停一下|别播了|关掉|关闭|停|不听了|不要了|够了)(?:音乐|歌曲?|歌|播放)?", "music_stop", 0),
    (r"(?:把)?(?:音乐|歌)(?:关了|关掉|停了|停掉)", "music_stop", 0),
    # 继续/恢复
    (r"(?:继续|继续播|接着放|接着播|恢复播放|继续放)(?:音乐|歌曲?|歌)?", "music_resume", 0),
    # 切歌
    (r"(?:下一首|切歌|换一首|跳过|下一曲|换一个|再来一首|不好听)", "music_next", 0),
    (r"(?:上一首|上一曲|前一首|再听一遍|重播)", "music_prev", 0),
    # 音量
    (r"(?:声音|音量)(?:大一?点|调大|加大|提高|太小了)", "music_vol_up", 0),
    (r"(?:声音|音量)(?:小一?点|调小|减小|降低|太大了|太吵)", "music_vol_down", 0),
    (r"(?:大声|大点声|大一点)", "music_vol_up", 0),
    (r"(?:小声|小点声|小一点|轻一点|轻点)", "music_vol_down", 0),
    # 推荐/每日推荐
    ...
    (r"(?:随便|随机)(?:播播|放放|听听|来点|放点|播|放|听)", "music_random", 0),
    (r"(?:我想)?听(?:点|些)?(?:歌|音乐)$", "music_random", 0),
    # 有具体歌名/歌手 → 搜索播放
    (r"(?:播放|放|听|来一?首|点一?首|播一?首)(?:一下)?(?:一首)?(.+?)(?:的歌|的音乐)?$", "music_play", 1),
    # 搜索
    (r"(?:搜|搜索|找)(?:一下)?(.+?)(?:的歌|的音乐|歌曲)$", "music_search", 1),
